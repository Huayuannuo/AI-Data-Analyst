...
def make_kb_collection_name(kb_name: str) -> str:
    """
    将任意知识库名称转换为合法 Milvus collection 名称。
    规则：
    - 前缀固定为 kb_
    - 仅保留 a-z0-9_
    - 其余字符转为下划线
    - 追加短哈希避免重名
    """
    raw = (kb_name or "").strip().lower()
    safe = re.sub(r"[^a-z0-9_]+", "_", raw)
    safe = re.sub(r"_+", "_", safe).strip("_")
    if not safe:
        safe = "kb"
    short_hash = hashlib.md5((kb_name or "").encode("utf-8")).hexdigest()[:8]
    return f"kb_{safe}_{short_hash}"
