...
class DocMindService:
    ...
    def _create_client(self) -> Optional[DocMindClient]:
        """创建 DocMind 客户端"""
        if not self.access_key_id or not self.access_key_secret:
            self.last_error = "DOCMIND_ACCESS_KEY_ID 或 DOCMIND_ACCESS_KEY_SECRET 未配置"
            return None
    ...
    def _safe_to_map(self, obj: Any) -> Dict[str, Any]:
        """将 SDK 响应对象尽量转换为 dict，便于完整日志输出。"""
        if obj is None:
            return {}
    ...
    def submit_job(self, file_path: str, file_name: str) -> Optional[str]:
        ...
        try:
            ...
            response_map = self._safe_to_map(response)
            print(f"[DocMind][submit_job] 原始响应: {response_map}")

            if response.body and response.body.data:
                task_id = response.body.data.id
                print(f"任务已提交，任务ID: {task_id}")
                return task_id
            body_map = self._safe_to_map(getattr(response, "body", None))
            code = body_map.get("Code") or body_map.get("code")
            msg = body_map.get("Message") or body_map.get("message")
            self.last_error = f"DocMind 返回空任务ID(code={code}, message={msg})"
            return None
    ...
    def query_status(self, task_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            response_map = self._safe_to_map(response)
            print(f"[DocMind][query_status] task_id={task_id}, 原始响应: {response_map}")
            if response.body and response.body.data:
                return response.body.data.to_map()
            body_map = self._safe_to_map(getattr(response, "body", None))
            code = body_map.get("Code") or body_map.get("code")
            msg = body_map.get("Message") or body_map.get("message")
            self.last_error = f"查询状态返回空数据(code={code}, message={msg})"
            return None
        except Exception as e:
            self.last_error = str(e)
            print(f"查询状态失败: {e}")
    ...
    def get_result(self, task_id: str, layout_num: int = 0, layout_step_size: int = 10) -> Optional[Any]:
        ...
        try:
            ...
            response_map = self._safe_to_map(response)
            print(
                f"[DocMind][get_result] task_id={task_id}, layout_num={layout_num}, "
                f"step={layout_step_size}, 原始响应: {response_map}"
            )
...
def process_document_with_docmind(
    ...
    try:
        ...
        if not task_id:
            result["message"] = f"文档提交失败: {service.last_error or '未知错误'}"
